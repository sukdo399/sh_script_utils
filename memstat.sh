...
while [ $value -gt 1024 ]
do
let value=`expr $value/1024`
let power=$power+1
value2=$(echo "${value2} 1024" | awk '{printf "%.3f", $1 / $2}')
done
fi
...
#shko fix
total=`awk -v column_number=5 '{ totalsum += $column_number} END { print totalsum }' /tmp/res2`
cp /tmp/res2 /tmp/res3
done
...
echo "---------------------------------------------------------------------------------------------"
echo
SLAB_CACHE=`cat /proc/meminfo | grep -i slab`
echo "${SLAB_CACHE}"
echo

echo "---------------------------------------------------------------------------------------------"
echo
SHR_MEM=0 && for LIST in `ipcs -m | egrep -v '^-|^key' | awk '{print $5}'`; do SHR_MEM=`expr ${SHR_MEM} + ${LIST}`; done && echo "Shared mem: ${SHR_MEM} kB"
echo
